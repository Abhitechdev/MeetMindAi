...
description: "A transparent look at the engineering pipeline behind MeetMind AI — from audio compression and Whisper transcription to LLM extraction and privacy-first architecture."
seoDescription: "Learn exactly how MeetMind AI converts raw meeting recordings into structured summaries, action items, and decisions using Whisper, Llama 3.3, and a privacy-first architecture."
...
![MeetMind AI Processing Flow Architecture](/images/blog/architecture.svg)
...
Because MeetMind AI outputs machine-readable structured data, every field can be independently consumed by downstream systems. Your action items are prepared for export to Jira (Coming Soon). Your decisions can be logged in Notion. Your executive summary can be easily shared to Slack channels. The structured format is what makes MeetMind AI a workflow integration, not just a note-taking app.
...
No. Your uploaded audio file is written to a temporary location on our processing server for transcription, and the temporary file is immediately deleted after processing. The original audio remains in your secure database account until you choose to delete it.

### What model do you use for summaries?

We use **Llama 3.3 70B Versatile**, a 70-billion parameter open-weight model hosted on Groq's LPU inference platform. It is accessed via a secure enterprise API, meaning your transcript data is not used to train the model.
...
Our architecture reflects our core philosophy: your meeting data belongs to you. By utilizing isolated transcription models and zero-retention LLMs, we deliver powerful insights without compromising your organization's security.
