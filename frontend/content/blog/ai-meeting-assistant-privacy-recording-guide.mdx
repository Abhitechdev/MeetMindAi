...
tags: ["AI Privacy", "Meeting Privacy", "Data Privacy", "AI Meeting Assistant", "Transcription"]
...
Every week, millions of professionals click "Record" on a meeting call. An [AI meeting assistant](/blog/how-meetmind-ai-works) silently joins — or waits for the uploaded file — and within seconds, a transcript appears. Summaries follow. Action items materialize. It feels effortless.
...
3. **Transcription:** A speech-to-text model (often based on OpenAI's Whisper or a proprietary ASR engine) converts the audio into text. For a deeper technical explanation, see our guide on [how AI transcription works](/blog/faster-whisper-explained).
4. **AI analysis:** A large language model processes the transcript to extract summaries, actionable commitments, decisions, and key topics.
...
| Architectural Dimension | Local On-Device Transcription (e.g., Faster-Whisper, whisper.cpp) | Commercial Cloud API Transcription |
| :--- | :--- | :--- |
| **Audio Traversal** | Stays strictly on local host machine / private network | Transmitted over encrypted TLS 1.2+ to provider API |
| **Hardware Requirement** | High (Requires dedicated GPU / 6–10GB VRAM for large models) | Minimal (Standard browser, mobile device, or lightweight laptop) |
| **Model Size Feasible** | Often restricted to `tiny` or `base` models on typical laptops | Full Whisper Large v3 or multi-billion parameter architectures |
| **Speaker Diarization** | High local compute overhead; complex speaker clustering | Server-side neural diarization (e.g. Deepgram Nova-3) |
| **Offline Capability** | Fully offline / air-gapped capability | Requires active internet connectivity |
| **Data Retention Risk** | Zero remote retention risk | Depends on vendor terms (must enforce ephemeral deletion) |

**Cloud-based transcription** sends audio over secure TLS to remote servers where dedicated accelerators (like Groq LPUs or cloud GPUs) process it in seconds. The trade-off is that data traverses external infrastructure, requiring strict contractual zero-training agreements and rapid unlinking.

**Local transcription** keeps audio entirely on the physical machine. The trade-off is that high-fidelity models (like Whisper Large v3 with 1.5B parameters) require dedicated GPU hardware. Running lightweight quantized models locally reduces memory, but introduces significantly higher Word Error Rates (WER) on accents and cross-talk.
...
- **No bot joins your meeting.** MeetMind AI does not inject a participant into your live call. You record using your native platform (Zoom, Google Meet, Teams) and upload the file afterward. This eliminates third-party bot presence and gives you full control over recording consent within your native video conferencing tool.
- **Ephemeral Audio Lifecycle:** Audio files are streamed to a temporary directory in 1MB chunks to prevent memory spikes. Immediately after the transcription API call finishes, our backend executes a cleanup routine that unlinks and permanently deletes the local audio file from the host server. We do not maintain long-term audio storage.
- **Zero Model Training on Customer Data:** We utilize commercial API endpoints (Groq Whisper Large v3 Turbo and Deepgram Nova-3 for speech-to-text; high-capacity language models for summarization). Under commercial developer terms, private audio payloads and transcripts are not used to train public foundation models.
- **Database Isolation via Row-Level Security:** Transcripts, summaries, action items, and decisions are stored in a managed PostgreSQL database with Row-Level Security (RLS) strictly enforced. Each user's data is only queryable by their authenticated user session.
- **Permanent User Deletion:** Users retain complete control. Deleting a meeting from the history dashboard executes a permanent hard delete of the transcript, summary, action items, and decisions from the active database.

What we do not claim: we do not execute on-device local transcription on the user's laptop. Processing runs on high-speed cloud infrastructure to ensure rapid transcription without exhausting client hardware.
